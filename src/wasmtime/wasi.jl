...
mutable struct WasiConfig
    ...
    WasiConfig(wasi_config_ptr, name) = finalizer(wasi_config_delete, new(wasi_config_ptr, name, false))
end
WasiConfig(program_name::String) = WasiConfig(wasi_config_new(), program_name)

Base.unsafe_convert(::Type{Ptr{wasi_config_t}}, wasi_config::WasiConfig) = wasi_config.wasi_config_ptr

wasi_config_new() = @ccall libwasmtime.wasi_config_new()::Ptr{wasi_config_t}
wasi_config_delete(wasi_config) = @ccall libwasmtime.wasi_config_delete(wasi_config::Ptr{wasi_config_t})::Cvoid
wasi_config_delete(config::WasiConfig) = config.owned || wasi_config_delete(config.wasi_config_ptr)

wasi_config_set_argv(config, argc, argv) =
    @ccall libwasmtime.wasi_config_set_argv(config::Ptr{wasi_config_t}, argc::Cint, argv::Ptr{Ptr{Cchar}})::Cvoid

wasi_config_inherit_argv(config) =
    @ccall libwasmtime.wasi_config_inherit_argv(config::Ptr{wasi_config_t})::Cvoid

wasi_config_set_env(config, envc, names, values) =
    @ccall libwasmtime.wasi_config_set_env(config::Ptr{wasi_config_t}, envc::Cint, names::Ptr{Ptr{Cchar}}, values::Ptr{Ptr{Cchar}})::Cvoid

wasi_config_inherit_env(config) =
    @ccall libwasmtime.wasi_config_inherit_env(config::Ptr{wasi_config_t})::Cvoid

wasi_config_set_stdin_file(config, path) =
    @ccall libwasmtime.wasi_config_set_stdin_file(config::Ptr{wasi_config_t}, path::Cstring)::Bool

wasi_config_inherit_stdin(config) =
    @ccall libwasmtime.wasi_config_inherit_stdin(config::Ptr{wasi_config_t})::Cvoid

wasi_config_set_stdout_file(config, path) =
    @ccall libwasmtime.wasi_config_set_stdout_file(config::Ptr{wasi_config_t}, path::Cstring)::Bool

wasi_config_inherit_stdout(config) =
    @ccall libwasmtime.wasi_config_inherit_stdout(config::Ptr{wasi_config_t})::Cvoid

wasi_config_set_stderr_file(config, path) =
    @ccall libwasmtime.wasi_config_set_stderr_file(config::Ptr{wasi_config_t}, path::Cstring)::Bool
...
wasi_config_preopen_dir(config, path, guest_path) =
    @ccall libwasmtime.wasi_config_preopen_dir(config::Ptr{wasi_config_t}, path::Cstring, guest_path::Cstring)::Bool
...
function WasiEnv(store, wasi_config)
    ...
    wasi_instance_ptr = wasi_instance_new(store, wasi_config.program_name, wasi_config, wasm_trap_ptr)
...
Base.unsafe_convert(::Type{Ptr{wasi_instance_t}}, wasi_env::WasiEnv) = wasi_env.wasi_instance_ptr

wasi_instance_new(store, name, config, trap_ptr) =
    @ccall libwasmtime.wasi_instance_new(store::Ptr{wasm_store_t}, name::Cstring, config::Ptr{wasi_config_t}, trap_ptr::Ptr{Ptr{wasm_trap_t}})::Ptr{wasi_instance_t}
...
function wasi_get_imports(_, wasm_module, wasi_env)
    ...
    wasi_externs = WasmVec{wasm_extern_vec_t, Ptr{wasm_extern_t}}(length(wasm_externs_ptr), pointer(wasm_externs_ptr))
