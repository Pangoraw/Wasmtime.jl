...
wasi_config_new(prg_name) = @ccall libwasmer.wasi_config_new(prg_name::Cstring)::Ptr{wasi_config_t}

wasi_config_inherit_stderr!(wasi_config) = @ccall libwasmer.wasi_config_inherit_stderr(wasi_config::Ptr{wasi_config_t})::Cvoid
wasi_config_inherit_stdin!(wasi_config) = @ccall libwasmer.wasi_config_inherit_stdin(wasi_config::Ptr{wasi_config_t})::Cvoid
wasi_config_inherit_stdout!(wasi_config) = @ccall libwasmer.wasi_config_inherit_stdout(wasi_config::Ptr{wasi_config_t})::Cvoid

wasi_config_capture_stdout!(wasi_config) = @ccall libwasmer.wasi_config_capture_stdout(wasi_config::Ptr{wasi_config_t})::Cvoid

mutable struct WasiEnv
    wasi_env_ptr::Ptr{wasi_env_t}

    WasiEnv(wasi_env_ptr::Ptr{wasi_env_t}) = finalizer(new(wasi_env_ptr)) do wasi_env
        wasi_env_delete(wasi_env)
    end
end
function WasiEnv(wasi_config_ptr::Ptr{wasi_config_t})
    wasi_env_ptr = @ccall libwasmer.wasi_env_new(wasi_config_ptr::Ptr{wasi_config_t})::Ptr{wasi_env_t}
